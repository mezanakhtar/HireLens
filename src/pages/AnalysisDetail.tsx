...
import {
  ArrowLeft, Download, FileText, Tag, TrendingUp, Zap, Map,
  Lightbulb, CheckCircle2, AlertCircle, CheckCheck, XCircle,
  Circle, Target, ChevronRight, Calendar, Loader2,
} from "lucide-react";
...
function KeywordBadge({ item }: { item: KeywordAnalysisItem }) {
  ...
  return (
    <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm ring-1 ${importanceRing}
      ${item.found
...
function PdfRing({ score, label, color }: { score: number; label: string; color: string }) {
  const size = 100; const r = size * 0.38; const cx = size / 2;
...
function PdfSection({ title, children, sectionId }: { title: string; children: React.ReactNode; sectionId?: string }) {
  return (
    ...
    >
      <div style={{ fontSize: 13, fontWeight: 700, color: "#1e40af", borderBottom: "2px solid #dbeafe", paddingBottom: 6, marginBottom: 12 }}>
        {title}
      </div>
      {children}
    </div>
...
function PdfReportLayout({ analysis }: { analysis: SavedAnalysis }) {
  const { result, targetRole, createdAt } = analysis;
  const date = createdAt.toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" });
  const foundKw = result.keywordAnalysis?.filter((k) => k.found) ?? [];
  const missingKw = result.keywordAnalysis?.filter((k) => !k.found) ?? [];
  const kwScore = result.keywordAnalysis?.length
    ? Math.round((foundKw.length / result.keywordAnalysis.length) * 100) : 0;

  const root: React.CSSProperties = { width: PDF_WIDTH, background: "#ffffff", fontFamily: "'Inter','Segoe UI',Arial,sans-serif", color: "#1e293b", padding: "60px 40px", boxSizing: "border-box" };
  const card: React.CSSProperties = { background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 10, padding: "16px 20px", marginBottom: 16 };

  return (
    <div style={root} data-pdf-root>
      {/* Header */}
      <div data-pdf-section="header" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 32 }}>
        <div>
          <div style={{ fontSize: 26, fontWeight: 900, color: "#1e40af", letterSpacing: "-0.5px", lineHeight: 1.1 }}>HireLens</div>
          <div style={{ fontSize: 13, fontWeight: 500, color: "#64748b", marginTop: 4, letterSpacing: "0.01em" }}>Resume Analysis Report</div>
      ...
      <div data-pdf-section="scores" style={{ ...card, display: "flex", gap: 32, justifyContent: "center", alignItems: "flex-start", flexWrap: "wrap" }}>
        ...
        {result.atsScore != null && <PdfRing score={result.atsScore} label="ATS Score" color={scoreColor(result.atsScore)} />}
        <PdfRing score={kwScore} label="Keyword Match" color={scoreColor(kwScore)} />
      </div>
        ...
        <PdfSection title="Section Scores" sectionId="section-scores">
          {result.sectionScores.map((s: SectionScore) => (
            <div key={s.section} style={{ marginBottom: 10 }}>
              ...
              {s.feedback && <div style={{ fontSize: 10, color: "#64748b", marginTop: 2, paddingLeft: 4 }}>{s.feedback}</div>}
            </div>
          ))}
        </PdfSection>
      )}
      ...
      <div data-pdf-section="strengths" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
        <PdfSection title="✓  Strengths">
          {result.strengths.map((s: string) => (
            <div key={s} style={{ display: "flex", gap: 6, marginBottom: 6, fontSize: 11 }}>
              <span style={{ flexShrink: 0, color: "#15803d" }}>•</span><span style={{ color: "#334155" }}>{s}</span>
            </div>
          ))}
        </PdfSection>
        <PdfSection title="!  Areas to Improve">
          {result.weaknesses.map((w: string) => (
            <div key={w} style={{ display: "flex", gap: 6, marginBottom: 6, fontSize: 11 }}>
              <span style={{ flexShrink: 0, color: "#dc2626" }}>•</span><span style={{ color: "#334155" }}>{w}</span>
            </div>
          ))}
        </PdfSection>
      </div>
      ...
      {/* Keywords */}
      <PdfSection title={`Keyword Analysis  (${foundKw.length} found / ${missingKw.length} missing)`} sectionId="keywords">
        <PdfBar label="Keyword Coverage" value={kwScore} color={scoreColor(kwScore)} />
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 12 }}>
          {result.keywordAnalysis.map((k: KeywordAnalysisItem) => (
            <div key={k.keyword} style={{ padding: "3px 8px", borderRadius: 6, fontSize: 10, fontWeight: 600, background: k.found ? "#dcfce7" : "#fee2e2", color: k.found ? "#15803d" : "#dc2626", border: `1px solid ${k.found ? "#bbf7d0" : "#fecaca"}` }}>
              {k.found ? "✓" : "✗"} {k.keyword}
        ...
        {missingKw.length > 0 && (
          <div style={{ marginTop: 12, background: "#fefce8", border: "1px solid #fde68a", borderRadius: 8, padding: "10px 14px" }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: "#92400e", marginBottom: 6 }}>Add these keywords to improve your ATS score:</div>
            <div style={{ fontSize: 11, color: "#78350f" }}>{missingKw.map((k) => k.keyword).join(" · ")}</div>
          </div>
        )}
      </PdfSection>
      ...
      <PdfSection title="Career Roadmap" sectionId="roadmap">
        {result.roadmap.map((phase: RoadmapPhase, i: number) => {
          const bc = phase.priority === "high" ? "#fca5a5" : phase.priority === "medium" ? "#fde68a" : "#93c5fd";
          const bg = phase.priority === "high" ? "#fff1f2" : phase.priority === "medium" ? "#fefce8" : "#eff6ff";
          return (
            <div key={phase.phase} style={{ display: "flex", gap: 12, marginBottom: 14 }}>
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#dbeafe", color: "#1e40af", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 12, flexShrink: 0, marginTop: 2 }}>{i + 1}</div>
              <div style={{ flex: 1, background: bg, border: `1.5px solid ${bc}`, borderRadius: 8, padding: "10px 14px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 100, fontWeight: 600, textTransform: "capitalize", background: phase.priority === "high" ? "#fee2e2" : phase.priority === "medium" ? "#fef9c3" : "#dbeafe", color: phase.priority === "high" ? "#dc2626" : phase.priority === "medium" ? "#b45309" : "#1e40af" }}>{phase.priority}</span>
                </div>
      ...
      <PdfSection title="Actionable Suggestions" sectionId="suggestions">
        {result.suggestions.map((s: string, i: number) => (
          <div key={s} style={{ display: "flex", gap: 10, marginBottom: 12 }}>
            <div style={{ width: 20, height: 20, borderRadius: "50%", background: "#dbeafe", color: "#1e40af", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 10, flexShrink: 0, marginTop: 1 }}>{i + 1}</div>
        ...
        <PdfSection title="Detected Keywords" sectionId="detected-keywords">
          <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
            {result.keywords.map((kw: string) => (
              <span key={kw} style={{ padding: "3px 10px", borderRadius: 100, fontSize: 10, fontWeight: 500, background: "#f1f5f9", color: "#475569", border: "1px solid #e2e8f0" }}>{kw}</span>
            ))}
          </div>
        </PdfSection>
      )}
...
export default function AnalysisDetail() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setFetchError(false);
    fetchSingleAnalysis(user.uid, id)
      .then((data) => {
  ...
  const handleDownloadPDF = useCallback(async () => {
    ...
    try {
      ...
      /* Measure section positions BEFORE html2canvas renders */
      ...
      const canvas = await html2canvas(el, {
        ...
        onclone: (clonedDoc) => {
          ...
          if (clonedEl) { clonedEl.style.background = "#ffffff"; clonedEl.style.color = "#1e293b"; }
        },
      });
      ...
      const makeSlice = (startPx: number, heightPx: number): HTMLCanvasElement => {
      ...
      if (sectionRects.length === 0) {
        ...
        for (let p = 0; p < totalPages; p++) {
          ...
          const sc = makeSlice(startPx, hPx);
      ...
      const role = analysis.targetRole && analysis.targetRole !== "General"
        ? `-${analysis.targetRole.replace(/\s+/g, "_")}` : "";
      pdf.save(`HireLens-Resume-Analysis${role}.pdf`);
      toast({ title: "PDF downloaded", description: "Your report has been saved." });
    } catch {
      toast({ title: "PDF failed", description: "Couldn't generate the PDF. Try again.", variant: "destructive" });
    } finally {
  ...
  const targetRole = analysis?.targetRole ?? "";
  ...
  /* ── Error / not found ── */
  if (fetchError || !analysis || !result) {
    return (
      <div className="container mx-auto px-4 py-20 max-w-4xl flex flex-col items-center gap-5 text-center">
        <AlertCircle className="h-12 w-12 text-destructive/60" />
        <div>
          <h2 className="text-xl font-semibold mb-1">Analysis not found</h2>
          <p className="text-sm text-muted-foreground">
            This analysis may have been deleted or doesn't belong to your account.
          </p>
        </div>
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div style={{ position: "absolute", left: -9999, top: 0, zIndex: -1 }} aria-hidden>
        <div ref={printRef}>
          <PdfReportLayout analysis={analysis} />
        </div>
      </div>
      ...
      <div className="flex items-start justify-between gap-4 mb-8 flex-wrap">
        <div className="space-y-1">
          <Button variant="ghost" size="sm" className="gap-2 -ml-2 mb-1 text-muted-foreground hover:text-foreground" asChild>
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            {analysis.createdAt.toLocaleDateString(undefined, { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
          </div>
        </div>
          ...
          <Tabs defaultValue="overview" className="w-full">
            ...
            <TabsContent value="roadmap" className="space-y-4 mt-0">
              <div className="flex items-center gap-3 p-4 rounded-xl bg-primary/5 border border-primary/20">
                ...
                <p className="text-sm text-muted-foreground">
                  Your personalized roadmap{targetRole && targetRole !== "General" ? ` for ${targetRole}` : ""} — prioritized steps to make your resume stand out.
                </p>
              </div>
            ...
            <TabsContent value="feedback" className="space-y-5 mt-0">
              ...
              {result.keywords.length > 0 && (
                <Card className="border-border/60">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-base flex items-center gap-2">
                      <Tag className="h-4 w-4 text-primary" /> All Detected Keywords
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="flex flex-wrap gap-2">
